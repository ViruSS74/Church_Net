...
{
    public class ViewsOfIDDocsService
    {
        ...
        public DataTable GetAllViews()
        {
            ...
            {
                string sql = "SELECT id as Id, name as Name FROM type_id_document ORDER BY name";
        ...
        public void SaveView(int? id, string name)
        {
            using (var conn = new SQLiteConnection(_connectionString))
            {
                conn.Open();
                string sql = id == null
                    ? "INSERT INTO type_id_document (name) VALUES (@name)"
                    : "UPDATE type_id_document SET name = @name WHERE id = @id";
        ...
        public void DeleteView(int id)
        {
            ...
            {
                conn.Open();
                string sql = "DELETE FROM type_id_document WHERE id = @id";
                using (var cmd = new SQLiteCommand(sql, conn))
                {
