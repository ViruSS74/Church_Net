...
namespace ChurchBudget.Forms
{
    public partial class IncomeForm : Form
    {
        ...
        public IncomeForm()
        {
            ...
            // ✅ ОБНОВЛЕННЫЙ ОБРАБОТЧИК ИЗМЕНЕНИЯ ДАТЫ
            dtpDocDate.ValueChanged += (s, e) =>
            {
                // Проверка существующих документов
                DateTime selectedDate = dtpDocDate.Value.Date;
                int docCount = _service.GetDocumentCountByDate("Income", selectedDate);

                if (docCount > 0)
                {
                    string message = $"На {selectedDate:dd.MM.yyyy} уже существует {docCount} документ(ов).\n\n" +
                                    "Продолжить ввод нового документа?";
            ...
            // Динамически формируем путь к базе
            ...
            // УДАЛИТЕ ВТОРОЙ ОБРАБОТЧИК ValueChanged (он дублирует)!
            // Было: dtpDocDate.ValueChanged += (s, e) => { txtDocNumber.Text = GenerateDocNumber(dtpDocDate.Value); };

            // Подписка на удаление строки
        ...
        private void IncomeForm_Load(object sender, EventArgs args)
        {
            ...
            DateTime currentDate = dtpDocDate.Value.Date;
            int docCount = _service.GetDocumentCountByDate("Income", currentDate);

            if (docCount > 0)
            {
                string message = $"На {currentDate:dd.MM.yyyy} уже существует {docCount} документ(ов).\n\n" +
                                "Продолжить ввод нового документа?";
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            {
                string message = $"На {selectedDate:dd.MM.yyyy} уже существует {docCount} документ(ов).\n\n" +
                                "Вы уверены, что хотите сохранить еще один?";
                ...
                using (var trans = conn.BeginTransaction())
                {
                  ...
                  VALUES ('ПКО', @pNum, @refId, @date, @amt, @base)";
                        using (var cmdPko = new SQLiteCommand(sqlPKO, conn, trans))
                        {
                            ...
                            cmdPko.Parameters.AddWithValue("@refId", docId);
                            ...
                            cmdPko.Parameters.AddWithValue("@base", "Рапортичка"); // ✅ ИСПРАВЛЕНО: просто "Рапортичка"
                        ...
                        // ✅ НОВОЕ СООБЩЕНИЕ С НОМЕРАМИ ОБОИХ ДОКУМЕНТОВ
                        string successMessage = $"Создан документ дохода № {txtDocNumber.Text}\n" +
                                                $"Создан ПКО № {pkoNumber}";

                        MessageBox.Show(successMessage, "Успешное сохранение", MessageBoxButtons.OK, MessageBoxIcon.Information);
