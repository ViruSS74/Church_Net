...
{
    public partial class ViewsOfIDDocsDirForm : Form
    {
        private ViewsOfIDDocsService _service = new ViewsOfIDDocsService();

        public ViewsOfIDDocsDirForm()
        {
            InitializeComponent();
            // Подписываем форму на событие загрузки
            this.Load += new EventHandler(ViewsOfIDDocsDirForm_Load);
        }

        private void ViewsOfIDDocsDirForm_Load(object sender, EventArgs e)
        {
            // 1. Загружаем данные
            LoadData();
        ...
        {
            var f = new ViewsOfIDDocsEditForm();
        ...
        private void btnEdit_Click(object sender, EventArgs e)
        {
            if (dgvData.CurrentRow != null)
            {
                ...
                var f = new ViewsOfIDDocsEditForm(id, name);
                if (f.ShowDialog() == DialogResult.OK)
                {
